...
public class Season3Fragment extends Fragment {
    ...
    RecyclerView rv_season03;

    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup parent, Bundle savedStateInstance) {
        View view = inflater.inflate(R.layout.fragment_season3, parent, false);

        rv_season03 = view.findViewById(R.id.rv_season03);
        ...
        call.enqueue(new Callback<ResponseList<Episode>>() {
            ...
            public void onResponse(Call<ResponseList<Episode>> call, Response<ResponseList<Episode>> response) {
                if(response.isSuccessful()){
                    rv_season03.setHasFixedSize(true);
                    rv_season03.setLayoutManager(new GridLayoutManager(getContext(), 1,RecyclerView.VERTICAL, false));

                    List<Episode> episodeListSeason03 = new ArrayList<>();
                    for(Episode e: response.body().getResults()){
                        String season = e.getEpisode().substring(0,3);
                        if(season.equals("S03")){
                            episodeListSeason03.add(e);
                        }
                    }
                    EpisodeAdapter episodeAdapter = new EpisodeAdapter((List<Episode>) episodeListSeason03, Season3Fragment.this);
                    rv_season03.setAdapter(episodeAdapter);
                }else{
