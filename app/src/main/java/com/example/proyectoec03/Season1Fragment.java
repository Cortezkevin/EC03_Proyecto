...
public class Season1Fragment extends Fragment {
    ...
    RecyclerView rv_season01;

    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup parent, Bundle savedStateInstance) {
        View view = inflater.inflate(R.layout.fragment_season1, parent, false);

        rv_season01 = view.findViewById(R.id.rv_season01);
        ...
        call.enqueue(new Callback<ResponseList<Episode>>() {
            ...
            public void onResponse(Call<ResponseList<Episode>> call, Response<ResponseList<Episode>> response) {
                if (response.isSuccessful()) {
                    rv_season01.setHasFixedSize(true);
                    rv_season01.setLayoutManager(new GridLayoutManager(getContext(), 1, RecyclerView.VERTICAL, false));

                    List<Episode> episodeListSeason01 = new ArrayList<>();
                    for (Episode e : response.body().getResults()) {
                        String season = e.getEpisode().substring(0, 3);
                        if (season.equals("S01")) {
                            episodeListSeason01.add(e);
                        }
                    }
                    EpisodeAdapter episodeAdapter = new EpisodeAdapter((List<Episode>) episodeListSeason01, Season1Fragment.this);
                    rv_season01.setAdapter(episodeAdapter);
                } else {
